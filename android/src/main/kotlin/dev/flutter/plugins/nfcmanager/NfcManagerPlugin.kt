...
class NfcManagerPlugin: FlutterPlugin, ActivityAware, PigeonHostApi, BroadcastReceiver() {
  ...
  override fun nfcAdapterEnableReaderMode(flags: List<PigeonReaderFlag>) {
  ...
  override fun nfcAdapterEnableForegroundDispatch() {
    ...
    getAdapter().enableForegroundDispatch(activity, pendingIntent, null, null)
  }
...
private fun toInt(value: List<PigeonReaderFlag>): Int {
  return value.fold(0) { p, e -> p or toInt(e)}
}

private fun toInt(value: PigeonReaderFlag): Int {
  return when (value) {
    PigeonReaderFlag.NFC_A-> NfcAdapter.FLAG_READER_NFC_A
    PigeonReaderFlag.NFC_B -> NfcAdapter.FLAG_READER_NFC_B
    PigeonReaderFlag.NFC_BARCODE -> NfcAdapter.FLAG_READER_NFC_BARCODE
    PigeonReaderFlag.NFC_F -> NfcAdapter.FLAG_READER_NFC_F
    PigeonReaderFlag.NFC_V -> NfcAdapter.FLAG_READER_NFC_V
    PigeonReaderFlag.NO_PLATFORM_SOUNDS -> NfcAdapter.FLAG_READER_NO_PLATFORM_SOUNDS
    PigeonReaderFlag.SKIP_NDEF_CHECK -> NfcAdapter.FLAG_READER_SKIP_NDEF_CHECK
  }
}
